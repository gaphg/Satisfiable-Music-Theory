  ...
  *)
  ...
  let checked_type, new_inferred =
    ...
    | Var name -> (
        ...
        | None, None, None ->
            raise
              (TypeInferenceError ("Unable to infer type of variable " ^ name))
    ...
    | And (e1, e2) | Or (e1, e2) | Implies (e1, e2) | Iff (e1, e2) ->
        ( BooleanType,
          accumulate_check
    ...
    | GreaterThanEq (e1, e2) ->
        check_lr_same e1 e2
    | EqualsModOctave (e1, e2) | NotEqualsModOctave (e1, e2) -> (
        ...
        | _ ->
            raise
              (TypeError
                 ("Expected operands of " ^ string_of_expr e
                ^ " to be of type Pitch or Interval")))
    | Flatten e -> (
        match expected with
        | Some ((PitchType | IntervalType) as t) ->
            (t, accumulate_check inferred e (Some t))
        | Some t ->
            raise
              (TypeError
                 ("Flatten returns type Pitch or Interval, but expected "
                ^ string_of_type t))
        | None -> (
            ...
            | _ ->
                raise
                  (TypeError
                     ("Expected " ^ string_of_expr e
                    ^ " to be of type Pitch or Interval, but got "
                    ^ string_of_type t))))
    | ElementAt (list, idx) -> (
        try
